...
```{r}
kids$j12_18_pre<-kids$J12_18
kids$j12_18_pre[kids$date>=as.Date('2010-03-01')]<-NA #set to missing for post period
kids$log.ach<-log(kids$ach_noj)

mod1<-glm(j12_18_pre~month+log.ach,family='quasipoisson', data=kids)
fit.j12_18.pre<-predict(mod1, type='response')

plot(kids$date, kids$J12_18, type='p', pch=16, bty='l', ylab="N hospitalizations", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_18)), main='Fit of synthetic control model')
points(kids$date[kids$date<as.Date('2010-03-01')],fit.j12_18.pre, type='l' ,col='#1b9e77' )
...
plot(kids$date, kids$J12_18, type='p', pch=16, bty='l', ylab="N hospitalizations", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_18)), main='Extrapolation to post-vaccine period')
points(kids$date,fit.j12_18.post, type='l' ,col='#7570b3' )
points(kids$date[kids$date<as.Date('2010-03-01')],fit.j12_18.pre, type='l' ,col='#1b9e77' )
abline(v=as.Date('2010-02-01'), lty=2, col='gray')
```
...
```{r}
ratio<-kids$J12_18/fit.j12_18.post
difference<-kids$J12_18-fit.j12_18.post
...
Broad interventions (e.g., social wlefare programs) will affect both the outcome and control disease. In this instance, we *want* to include the control variables that are influenced by the intervention because it will help to adjust for trends in the outcome variable that are unrelated to vaccination.
...
To evaluate changes in disease rates associated with an intervention, the first step is to format the data into a **time series**. Time series enumerate the number of cases in a time period (e.g., week, month, quarter, or year). We typically do this by starting with a spreadsheet that has individual-level data, creating a variable that has the date rounded down to the date of the beginning of the nearest week/month/quarter/year, and then adding up the number of cases that occurred during that time period. 
...
If you have multiple strata, such as different age groups or regions, the dataset will need to reflect this. In the table below, we have 3 age groups, each with 5 observations. Each date/age group combination should have only 1 row in this dataset, *and all date/age combinations should be represented, even if no cases were observed during a particular time period*.
```{r, echo=F}
...
In the *exercise*, we will learn how to take a individual-level dataset that has 1 observation for each hospitalization/death and convert it to a time series dataset using the statistical software R.

TODO: USE EXAMPLE FROM PH SURVEILLANCE CLASS WITH CHILE DATA, FORMATTING DATA< EXPLORING ICD CODES

-	Descriptive analysis
The most useful thing to do to ensure the data are approriately formatted is to plot the data as a time series line plot, where we have time on the x-axis and number of cases on the y-axis. 
```{r, echo=F}
ds.sub<-dataset[dataset$age_group==1,]
plot(ds.sub$date,ds.sub$pneumonia, type='l', bty='l', col='blue')
```
