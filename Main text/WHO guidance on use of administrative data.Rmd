...
kids<-ecuador_mortality[ecuador_mortality$age_group=='ec 2-59m A',]
kids<-kids[kids$monthdate<=as.Date('2015-12-01'),]
...
##	Types of analyses
In this context, a 'time series' is defined as a variable in which the number of cases is tallied in each unit of time (week/month/quarter/year). The goal for the analysis is to detect changes in the average number of cases or incidence. In this example, we have the number of deaths coded as having a cause of pneumonia (ICD10 codes J12-J18) in Ecuador in children aged 2-59 months

```{r, fig.wdith=7, fig.height=3}
plot(kids$date, kids$J12_J18_prim, type='l', bty='l', ylab="N Deaths", xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='Time series of pneumonia deaths')
abline(v=as.Date('2010-08-01'), lty=2, col='gray')
```
...
```{r, fig.wdith=7, fig.height=3}
mean.pre<-mean(kids$J12_J18_prim[kids$date<as.Date('2010-08-01')])
mean.post<-mean(kids$J12_J18_prim[kids$date>=as.Date('2011-08-01')])

plot(kids$date, kids$J12_J18_prim, type='l', bty='l', ylab="N Deaths", xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='Pre-post comparison')
abline(v=as.Date('2010-08-01'), lty=2, col='gray')
arrows(x0=as.Date('2005-01-01'), x1=as.Date('2010-07-01') , y0=mean.pre, length=0, col='#1b9e77')
arrows(x0=as.Date('2011-08-01'), x1=as.Date('2015-12-01') , y0=mean.post, length=0, col='#7570b3')
...
plot(kids$date, kids$J12_J18_prim, type='p',pch=16,cex=0.75, bty='l',col='lightgray', ylab="N Deaths", xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='Counterfactual for pre/post comparisons')
arrows(x0=as.Date('2005-01-01'), x1=as.Date('2010-07-01') , y0=mean.pre, length=0, col='#1b9e77')
arrows(x0=as.Date('2011-08-01'), x1=as.Date('2016-01-01') , y0=mean.post, length=0, col='#7570b3')
arrows(x0=as.Date('2011-08-01'), x1=as.Date('2016-01-01') , y0=mean.pre, length=0, col='#d95f02', lty=3)
abline(v=as.Date('2010-08-01'), lty=3, col='gray')
abline(v=as.Date('2011-08-01'), lty=3, col='gray')
text(as.Date('2016-01-01'), 13000, 'Counterfactual', col='#d95f02', cex=0.8,pos=2, offset=0)
text(as.Date('2016-01-01'), 8000, 'Fitted', col='#7570b3', cex=0.8,pos=2, offset=0)
...
kids$period<-0
kids$period[kids$date>=as.Date('2010-08-01')  ] <-1
kids$period[kids$date>=as.Date('2011-08-01')  ]<-2
mod1<-glm(J12_J18_prim~month+as.factor(period),family='quasipoisson', data=kids )
...
pred1.counter.post[kids$date<=as.Date('2011-08-01')] <-NA

pred1.pre<-pred1
pred1.pre[kids$date>=as.Date('2010-08-01')]<-NA

pred1.post<-pred1
pred1.post[kids$date<as.Date('2011-08-01')]<-NA


par(mfrow=c(2,1))
plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)),main='Pre/post comparison with adjustment for season' )
...
mod1<-glm(J12_J18_prim~month+as.factor(period)*index,family='quasipoisson', data=kids ,offset=log.offset)
pred1<-predict(mod1, type='response')
kid.counter<-kids
kid.counter$period<-as.factor(0)
pred1.counter<-predict(mod1, type='response', newdata=kid.counter)
...
par(mfrow=c(2,1))
plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='ITS, with disjointed segments')
points(kids$date,pred1.pre, type='l' ,col='#1b9e77' )
points(kids$date,pred1.post, type='l' , col='#7570b3' )
points(kids$date,pred1.counter, type='l', col='#d95f02', lty=3  )
abline(v=as.Date('2010-08-01'), lty=3, col='gray')
...
```{r, fig.width=5, fig.height=7}
kids$index<-1:nrow(kids) #creates an index for time
intro.date<-min(which(kids$date>=as.Date('2010-08-01')))
eval.date<-min(which(kids$date>=as.Date('2011-08-01')))
...
mod1<-glm(J12_J18_prim~month+ index +spl1 + spl2 ,family='quasipoisson', data=kids,offset=log.offset )
pred1<-predict(mod1, type='response')
kid.counter<-kids
kid.counter$spl1<-0
kid.counter$spl2<-0
...
par(mfrow=c(2,1))
plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='ITS, with connected segments')
points(kids$date,pred1.pre, type='l' ,col='#1b9e77' )
points(kids$date,pred1.post, type='l' , col='#7570b3' )
points(kids$date,pred1.counter, type='l', col='#d95f02', lty=3  )
abline(v=as.Date('2010-08-01'), lty=3, col='gray')
...
This could be further modified to allow the slope to level out after a certain time period (in this example, the post-vaccine trend levels out 24 months after vaccine introduction)
...
pred1.post<-pred1
pred1.post[kids$date<as.Date('2010-08-01')]<-NA

par(mfrow=c(2,1))
plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='ITS, with leveling of the trend')
points(kids$date,pred1.pre, type='l' ,col='#1b9e77' )
points(kids$date,pred1.post, type='l' , col='#7570b3' )
points(kids$date,pred1.counter, type='l', col='#d95f02', lty=3  )
abline(v=as.Date('2010-08-01'), lty=3, col='gray')
...
intro.date<-min(which(kids$date>=as.Date('2010-03-01')))
eval.date<-min(which(kids$date>=as.Date('2011-03-01')))
kids$log.offset.acm<- log(kids$acm_noj_prim)
kids$spl1<- kids$index -  intro.date
kids$spl2<- kids$index -  eval.date
kids$spl1[kids$spl1>12]<-12 #effect only in place for 12 months
kids$spl2[kids$spl2>24]<-24 #Level off after 24 months
...
mod1<-glm(J12_J18_prim~month+ index +spl1 + spl2 +log.offset.acm,family='quasipoisson', data=kids )
pred1<-predict(mod1, type='response')
kid.counter<-kids
kid.counter$spl1<-0
kid.counter$spl2<-0
...
par(mfrow=c(2,1))
plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='ITS, adjusted for ACM')
points(kids$date,pred1.pre, type='l' ,col='#1b9e77' )
points(kids$date,pred1.post, type='l' , col='#7570b3' )
points(kids$date,pred1.counter, type='l', col='#d95f02', lty=3  )
abline(v=as.Date('2010-08-01'), lty=3, col='gray')
...
# plot(kids$date, kids$J12_J18_prim/kids$acm_noj_prim, type='p', pch=16, bty='l', ylab="Adj N hospitalizations", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim/kids$acm_noj_prim)), main='ITS, adjusted')
# points(kids$date,pred1.pre/kids$acm_noj_prim, type='l' ,col='#1b9e77' )
# points(kids$date,pred1.post/kids$acm_noj_prim, type='l' , col='#7570b3' )
# points(kids$date,pred1.counter/kids$acm_noj_prim, type='l', col='#d95f02', lty=3  )
# abline(v=as.Date('2010-08-01'), lty=3, col='gray')
# abline(v=as.Date('2011-08-01'), lty=3, col='gray')
...
### Synthetic controls
With the synthetic controls approach, the goal is to use time series that are not influenced by the vaccine to adjust for underlying secular trends in the data. With the "control covariates" approach described above, the control disease is included as an adjustment in the regression model, and the model is set up to detect changes in slope or level of disease after introduction of the vaccine. Synthetic controls analysis is an extenstion of this idea. The main differences are (1) that the models are only fit to data from the pre-vaccine period and used to extrapolate to the post-vaccine period and (2) typically many control time series are included, and variable selection is used to weight these controls. 

**Simple example with a single time series**
Using the same dataset from Ecuador that was used in the ITS example, we will evaluate changes in J12-J18 following introduction of the vaccine. We will fit the model just to data form the pre-vaccine period, adjusting for seaosnality and the control variable. As an example, we will use log(all-cause)
```{r}
kids$j12_18_pre<-kids$J12_J18_prim
kids$j12_18_pre[kids$date>=as.Date('2010-08-01')]<-NA #set to missing for post period
kids$log.ach<-log(kids$acm_noj_prim)

mod1<-glm(j12_18_pre~month+log.ach,family='quasipoisson', data=kids)
fit.j12_18.pre<-predict(mod1, type='response')

plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='Fit of synthetic control model')
points(kids$date[kids$date<as.Date('2010-08-01')],fit.j12_18.pre, type='l' ,col='#1b9e77' )
...
```{r}
fit.j12_18.post<-predict(mod1, type='response', newdata=kids)

plot(kids$date, kids$J12_J18_prim, type='p', pch=16, bty='l', ylab="N Deaths", col='gray', cex=0.7,  xlab='date', ylim=c(0, max(kids$J12_J18_prim)), main='Extrapolation to post-vaccine period')
points(kids$date,fit.j12_18.post, type='l' ,col='#d95f02' )
points(kids$date[kids$date<as.Date('2010-08-01')],fit.j12_18.pre, type='l' ,col='#1b9e77' )
abline(v=as.Date('2010-07-01'), lty=2, col='gray')
```
...
```{r}
eval.start<-as.Date('2011-08-01')
evalstart.index<-which(kids$date==eval.start)
evalstart.index<-evalstart.index:nrow(kids)
ratio<-kids$J12_J18_prim/fit.j12_18.post
ave.ratio<- sum(kids$J12_J18_prim[evalstart.index])/sum(fit.j12_18.post[evalstart.index])
difference<-kids$J12_J18_prim-fit.j12_18.post


plot(kids$date,ratio, type='l', bty='l', main='Ratio')
...
plot(ds.sub$monthdate,ds.sub$J12_J18_prim, type='l', bty='l', col='blue', xlab='Date', ylab='J12-18', ylim=c(0, max(ds.sub$J12_J18_prim)))

plot(ds.sub$monthdate,ds.sub$acm_noj_prim, type='l', bty='l', col='blue', xlab='Date', ylab='All non-resp.', ylim=c(0, max(ds.sub$acm_noj_prim)))
```
