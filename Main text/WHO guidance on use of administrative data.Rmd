...
<<TODO INSERT FIG 1 HERE (Spectrum of Admin Data)>>
...
b.	Then you enter the page below, where you should follow the steps
 <<TODO: insert fig screenshot of WHO website here>>
...
intro.date<-as.Date('2010-08-01')
eval.date<-as.Date('2011-08-01')
...
```{r, fig.width=5, fig.height=3}
mean.pre<-mean(kids$J12_J18_prim[kids$date< intro.date])
mean.post<-mean(kids$J12_J18_prim[kids$date>=eval.date])
...
arrows(x0=eval.date, x1=as.Date('2015-12-01') , y0=mean.post, length=0, col='#7570b3')
...
arrows(x0=as.Date('2005-01-01'), x1=as.Date('2010-07-01') , y0=mean.pre, length=0, col='#1b9e77')
arrows(x0=eval.date, x1=as.Date('2016-01-01') , y0=mean.post, length=0, col='#7570b3')
arrows(x0=eval.date, x1=as.Date('2016-01-01') , y0=mean.pre, length=0, col='#d95f02', lty=3)
...
Let's just compare the Rate ratio estimates from the different ITS models we have tested so far. 
```{r}

its.comp<-cbind.data.frame(its.rr.control, its.rr.connected1, its.rr.disjoint, its.rr.level,rr.pre.post)
matplot(its.comp, type='l', bty='l')
legend('bottomleft', legend=c("ITS:control", "ITS:smooth","ITS:Disjointed",'ITS:plateau', 'Pre/Post'),
        cex=0.8,
       box.lty=0)
```
...
kids$j12_18_pre[kids$date>=intro.date]<-NA #set to missing for post period
...
points(kids$date[kids$date<intro.date],fit.j12_18.pre, type='l' ,col='#1b9e77' )
...
•	Published papers using administrative data to assesws PCV in Africa
